...
this.editor=this.app.find("."+a.editorClassName).attr("contenteditable","true").attr("tabindex",0);c.each(["dir","spellcheck","autocomplete","autocorrect","autocapitalize"],c.proxy(function(b,d){this.editor.attr(d,a[d])},this));this.filters=this.app.find("."+a.filtersClassName);a.autoHideFilters&&this.filters.hide();this.tabs=this.app.find("."+a.tabsClassName);c.each(a.filters,c.proxy(function(b,d){c("<span/>").wrapInner(w(d.icon,'<span class="emojione-{unicode}">{alt}</span>')).addClass(a.filterClassName).attr("data-filter",
b).attr("role","button").appendTo(this.filters);c("<div/>").wrapInner(w(d.emoji,'<span class="emojibtn"><span class="emojione-{unicode}" data-shortname="{shortname}">{alt}</span></span>')).addClass(a.tabClassName).addClass(a.tabClassName+"-"+b).hide().appendTo(this.tabs)},this));this.app.insertAfter(this.element);this.element.hide();this.setText(this.element[this.elementValFunc]())}function B(a){a=c.extend({},C,a);A.apply(this,[a]);h.apply(this,[this.filters,{mousedown:"emojioneArea.filters.mousedown filters.mousedown"},
...
a.tabClassName+"-"+b.data("filter")).show())}).on("emojioneArea.paste",function(a,d){var n="emojionearea-paster-"+(new Date).getTime(),e=c(f);q('<span id="'+n+'">&nbsp;</span>');var k=s(a[0]),g=c(f).scrollTop(),h=a.scrollTop(),l=c("<div />").appendTo(c("BODY"));this.stayFocused=!0;l.attr("contenteditable","true").attr("tabindex","0").css({position:"fixed",left:"-9999px",width:"1px",height:"1px",top:10}).focus();e.scrollTop(g);a.removeAttr("contenteditable");setTimeout(c.proxy(function(){a.attr("contenteditable",
"true").focus();x(a[0],k);q(y(v(l.html())));q('<span id="'+n+'-caret"></span>');e.scrollTop(g);a.scrollTop(h);var d=c("#"+n+"-caret"),f=d.offset().top-a.offset().top,m=a.height();(h+f>=m||h>f)&&a.scrollTop(h+f-2*m/3);d.remove();l.remove();this.stayFocused=!1},this),200)}).on("emojioneArea.emojibtn.click",function(a){s(this.editor[0]);q(w(a.children("span").data("shortname"),'<img class="emojione" alt="{alt}" src="{imagePng}"/>'))}).on("emojioneArea.filters.mousedown emojioneArea.tabs.mousedown",function(b,
...
c.proxy(function(a,c){c===b&&(this.events[e]=this.events[e].splice(a,1))},this))},this)):c.each(a.split(" "),c.proxy(function(a,b){d.test(b)||(this.events[b]=[])},this))}return this};p.prototype.setText=function(a){this.editor.html("<div>"+y(a)+"</div>");this.content=this.editor.html();t.apply(this,["emojioneArea.change change",this.content])};p.prototype.getText=function(){return v(this.editor.html())};var r={};c.each(g.emojioneList,function(a,b){r[a.replace("-","_")]=b});c.fn.emojioneArea=function(a){return this.each(function(){return this.emojioneArea?
this.emojioneArea:this.emojioneArea=new p(c(this),a)})}})(document,window,jQuery,emojione);
