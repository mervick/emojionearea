...
1),c.each(a.toLowerCase().split(" "),c.proxy(function(a,e){this.events[e]&&this.events[e].length&&c.each(this.events[e],c.proxy(function(a,c){return b=!1!==c.apply(this,d)},this));return b},this)));return b}function h(a,b,d){function n(b,a,e){b.on(a,c.proxy(function(){var b=c.isFunction(d)?d.apply(this,[a].concat(u.call(arguments))):d;b&&t.apply(this,[e].concat([b]).concat(u.call(arguments)))},this))}function e(a){"string"==(typeof b).toLowerCase()?n.apply(this,[a,b,b]):c.each(b,c.proxy(function(d,
e){n.apply(this,[a,c.isArray(b)?e:d,e])},this))}d=d||function(b,a){return c(a.currentTarget)};c.isArray(a)?c.each(a,c.proxy(function(b,a){e.apply(this,[c(a)])},this)):e.apply(this,[a])}function y(a){return z(a.replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/(?:\r\n|\r|\n)/g,"\n").replace(/(\n+)/g,"<div>$1</div>").replace(/\n/g,"<br/>").replace(/<br\/><\/div>/g,"</div>")).replace(/\t/g,"&nbsp;&nbsp;&nbsp;&nbsp;").replace(/  /g,"&nbsp;&nbsp;")}function v(a){return a.replace(/<img[^>]*alt="([^"]+)"[^>]*>/ig,
...
"\n\n").replace(/<(?:[^>]+)?>/g,"").replace("&nbsp;"," ")}function z(a){return a.replace(RegExp("<object[^>]*>.*?</object>|<span[^>]*>.*?</span>|<(?:object|embed|svg|img|div|span|p|a)[^>]*>|("+g.unicodeRegexp+")","gi"),function(b){if("undefined"===typeof b||""===b||!(b in g.jsecapeMap))return b;b=g.jsecapeMap[b];return'<img class="emojione" alt="'+g.convert(b)+'" src="'+g.imagePathPNG+b+".png"+g.cacheBustParam+'"/>'})}function w(a,b){return a.replace(/:?[\w_]+:?,?/g,function(a){a=":"+a.replace(/,$/,
"").replace(/:$/,"").replace(/^:/,"")+":";if(a in r){var c=r[a][r[a].length-1].toUpperCase(),e=g.convert(c);return b.replace("{shortname}",a).replace("{imagePng}",g.imagePathPNG+c+".png"+g.cacheBustParam).replace("{unicode}",c).replace("{alt}",e)}return a})}function A(a){this.app=a.template;c.each(["editor","filters","tabs"],c.proxy(function(b,d){this.app=this.app.replace(RegExp("<"+d+"/?>","i"),'<div class="'+a[d+"ClassName"]+'"></div>')},this));this.app=c("<div>"+this.app+"</div>").addClass(a.className).attr("role",
"application");this.editor=this.app.find("."+a.editorClassName).attr("contenteditable","true").attr("tabindex",0);(this.placeholder=a.placeholder||this.element.data("placeholder")||this.element.attr("placeholder"))&&this.editor.attr("placeholder",this.placeholder);c.each(["dir","spellcheck","autocomplete","autocorrect","autocapitalize"],c.proxy(function(b,d){this.editor.attr(d,a[d])},this));this.filters=this.app.find("."+a.filtersClassName);a.autoHideFilters&&this.filters.hide();this.tabs=this.app.find("."+
a.tabsClassName);c.each(a.filters,c.proxy(function(b,d){c("<span/>").wrapInner(w(d.icon,'<span class="emojione-{unicode}">{alt}</span>')).addClass(a.filterClassName).attr("data-filter",b).attr("role","button").appendTo(this.filters);c("<div/>").wrapInner(w(d.emoji,'<span class="emojibtn"><span class="emojione-{unicode}" data-shortname="{shortname}">{alt}</span></span>')).addClass(a.tabClassName).addClass(a.tabClassName+"-"+b).hide().appendTo(this.tabs)},this));this.app.insertAfter(this.element);this.element.hide();
...
u=[].slice,s,x,r={};c.each(g.emojioneList,function(a,b){r[a.replace("-","_")]=b});m.getSelection&&f.createRange?(s=function(a){var b=m.getSelection().getRangeAt(0),d=b.cloneRange();d.selectNodeContents(a);d.setEnd(b.startContainer,b.startOffset);a=d.toString().length;return{start:a,end:a+v(c("<div/>").append(b.extractContents()).html()).length}},x=function(a,b){var d=0,c=f.createRange();c.setStart(a,0);c.collapse(!0);for(var e=[a],k,g=!1,h=!1;!h&&(k=e.pop());)if(3==k.nodeType){var l=d+k.length;!g&&
(b.start>=d&&b.start<=l)&&(c.setStart(k,b.start-d),g=!0);g&&(b.end>=d&&b.end<=l)&&(c.setEnd(k,b.end-d),h=!0);d=l}else for(l=k.childNodes.length;l--;)e.push(k.childNodes[l]);d=m.getSelection();d.removeAllRanges();d.addRange(c)}):f.selection&&f.body.createTextRange&&(s=function(a){var b=f.selection.createRange(),d=f.body.createTextRange();d.moveToElementText(a);d.setEndPoint("EndToStart",b);a=d.text.length;return{start:a,end:a+b.text.length}},x=function(a,b){var d=f.body.createTextRange();d.moveToElementText(a);
d.collapse(!0);d.moveEnd("character",b.end);d.moveStart("character",b.start);d.select()});var p=function(a,b){this.element=c(a);this.elementValFunc=this.element.is("INPUT")?"val":"text";this.events={};B.apply(this,[b])};p.prototype.on=function(a,b){a&&c.isFunction(b)&&c.each(a.toLowerCase().split(" "),c.proxy(function(a,c){this.events[c]||(this.events[c]=[]);this.events[c].push(b)},this));return this};p.prototype.off=function(a,b){if(a){var d=/^emojioneArea\./;c.isFunction(b)?c.each(a.toLowerCase().split(" "),
c.proxy(function(a,e){!d.test(e)&&(this.events[e]&&this.events[e].length)&&c.each(this.events[e],c.proxy(function(a,c){c===b&&(this.events[e]=this.events[e].splice(a,1))},this))},this)):c.each(a.split(" "),c.proxy(function(a,b){d.test(b)||(this.events[b]=[])},this))}return this};p.prototype.setText=function(a){this.editor.html('<div class="placeholder-fix">'+y(a)+"</div>");this.placeholder&&this.editor.children(".placeholder-fix:first").attr("placeholder",this.placeholder);this.content=this.editor.html();
t.apply(this,["emojioneArea.change change",this.content])};p.prototype.getText=function(){return v(this.editor.html())};c.fn.emojioneArea=function(a){return this.each(function(){return this.emojioneArea?this.emojioneArea:this.emojioneArea=new p(c(this),a)})}})(document,window,jQuery,emojione);
