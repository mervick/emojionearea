(function(h,q,f){function w(c){k?c():y.push(c)}function E(c){var a,b;if(q.getSelection){if(a=q.getSelection(),a.getRangeAt&&a.rangeCount){b=a.getRangeAt(0);b.deleteContents();var d=h.createElement("div");d.innerHTML=c;c=h.createDocumentFragment();for(var f,e;f=d.firstChild;)e=c.appendChild(f);b.insertNode(c);e&&(b=b.cloneRange(),b.setStartAfter(e),b.collapse(!0),a.removeAllRanges(),a.addRange(b))}}else h.selection&&"Control"!=h.selection.type&&h.selection.createRange().pasteHTML(c)}function F(c,a,
b){var d=!0,p=1;if(a){a=a.toLowerCase();b=b||[];do{var e=1==p?"@"+a:a;m[c.id][e]&&m[c.id][e].length&&f.each(m[c.id][e],function(a,e){return d=!1!==e.apply(c,b)})}while(d&&p--)}return d}function R(c,a){return f(a.currentTarget)}function r(c,a,b,d){function p(a,b,e){a.on(b,function(){var a=f.isFunction(d)?d.apply(c,[b].concat(K.call(arguments))):d;a&&F(c,e,[a].concat(K.call(arguments)))})}function e(a){"string"===(typeof b).toLowerCase()?p(a,b,b):f.each(b,function(c,d){p(a,f.isArray(b)?d:c,d)})}d=d||
R;f.isArray(a)?f.each(a,function(a,b){e(f(b))}):e(a)}function L(c,a){return S(c.replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/(?:\r\n|\r|\n)/g,"\n").replace(/(\n+)/g,"<div>$1</div>").replace(/\n/g,"<br/>").replace(/<br\/><\/div>/g,"</div>"),'<img alt="{alt}" class="emojione'+(a.sprite?'-{uni}" src="'+M+'">':'" src="{img}">')).replace(/\t/g,"&nbsp;&nbsp;&nbsp;&nbsp;").replace(/  /g,"&nbsp;&nbsp;")}function H(c){return c.replace(/<img[^>]*alt="([^"]+)"[^>]*>/ig,"$1").replace(/\n|\r/g,"").replace(/<br[^>]*>/ig,
...
" ").replace(/\x20\x20/g,"&nbsp; ").replace(/\x20\x20/g," &nbsp;")}function N(c,a,b){return c.replace("{name}",b||"").replace("{img}",k.imagePathPNG+a+".png").replace("{uni}",a).replace("{alt}",k.convert(a))}function S(c,a){return c.replace(O,function(b){return"undefined"!==typeof b&&b in k.jsecapeMap?N(a,k.jsecapeMap[b]):b})}function I(c,a){return c.replace(/:?[\w_]+:?/g,function(b){b=":"+b.replace(/:$/,"").replace(/^:/,"")+":";return b in u?N(a,u[b][u[b].length-1].toUpperCase(),b):b})}function T(c,
a){function b(){x||(f.each(z,function(a,b){x+=f(b).outerWidth(!0)}),u=z.eq(0).outerWidth(!0));x>A?(B.addClass("active"),C.addClass("active"),n+x<=A?(n=A-x,B.removeClass("active")):0<=n&&(n=0,C.removeClass("active")),t.css("left",n)):(0!==n&&(n=0,t.css("left",n)),B.removeClass("active"),C.removeClass("active"))}a=f.extend({},U,a);var d=this,p=c.is("TEXTAREA")||c.is("INPUT")?"val":"text",e=a.template,h=!1,k=a.container?f(a.container):!1,g,l,v,t,z,C,B,A,n=0,x=0,u,w=function(){var a=l.width();a!==A&&
(A=a,F(d,"resize",[g]))},y;d.id=(new Date).getTime();d.placeholder=a.placeholder||c.data("placeholder")||c.attr("placeholder")||"";d.sprite=a.useSprite;m[d.id]={};for(var D=["editor","filters","tabs"],s=0;3>s;s++)e=e.replace(RegExp("<"+D[s]+"/?>","i"),'<div class="emojionearea-'+D[s]+'"></div>');e=f("<div>"+e+"</div>").addClass("emojionearea").attr("role","application");this.editor=e.find(".emojionearea-editor").attr("contenteditable","true").attr("placeholder",d.placeholder).attr("tabindex",0);g=
d.editor;D=["dir","spellcheck","autocomplete","autocorrect","autocapitalize"];for(s=0;5>s;s++)g.attr(D[s],a[D[s]]);l=e.find(".emojionearea-filters");a.autoHideFilters&&l.hide();v=e.find(".emojionearea-tabs");f.each(a.filters,function(a,b){f("<i/>").wrapInner(I(b.icon,d.sprite?'<i class="emojione-{uni}"/>':'<img class="emojione" src="{img}"/>')).addClass("emojionearea-filter").data("filter",a).appendTo(l);f("<div/>").data("items",I(b.emoji,'<i class="emojibtn" role="button">'+(d.sprite?'<i class="emojione-{uni}"':
'<img class="emojione" src="{img}"')+' data-name="{name}"/></i>')).addClass("emojionearea-tab").addClass("emojionearea-tab-"+a).hide().appendTo(v)});l.wrapInner('<div class="emojionearea-filters-scroll"/>');C=f('<i class="emojionearea-filter-arrow-left"/>').attr("role","button").appendTo(l);B=f('<i class="emojionearea-filter-arrow-right"/>').attr("role","button").appendTo(l);z=l.find(".emojionearea-filter");t=l.children(".emojionearea-filters-scroll");k?k.wrapInner(e):e.insertAfter(c);a.hideSource&&
c.hide();d.setText(c[p]());r(d,[l,v],{mousedown:"area.mousedown"},g);r(d,g,"paste",g);r(d,g,["focus","blur"],function(){return h?!1:g});r(d,[g,l,v],["mousedown","mouseup","click","keyup","keydown"],g);r(d,l.find(".emojionearea-filter"),{click:"filter.click"});r(d,C,{click:"arrowLeft.click"});r(d,B,{click:"arrowRight.click"});d.on("@filter.click",function(a){if(a.is(".active"))a.removeClass("active"),v.children().hide();else{z.filter(".active").removeClass("active");a.addClass("active");var b,c,e=
v.children().hide().filter(".emojionearea-tab-"+a.data("filter")).show(),f=e.data("items"),g={click:"emojibtn.click"};f&&(e.data("items",!1),f=f.split(","),d.sprite?(e.html(f.join("")),r(d,e.find(".emojibtn"),g)):c=P(function(){for(b=0;20>b&&f.length;b++)e.append(f.shift()),r(d,e.find(".emojibtn").not(".handled").addClass("handled"),g);f.length||Q(c)},5))}}).on("@resize",function(){b()}).on("@arrowLeft.click",function(){n+=u;b()}).on("@arrowRight.click",function(){n-=u;b()}).on("@paste",function(a){h=
!0;E("<span> </span>");var b=G(a[0]),c=a.scrollTop(),e=f("<div/>").appendTo(f("BODY")).attr("contenteditable","true").css({position:"fixed",left:"-999px",width:"1px",height:"1px",top:"20px",overflow:"hidden"}).focus();q.setTimeout(function(){var g="caret-"+(new Date).getTime();a.focus();J(a[0],b);E(L(H(e.html().replace(/\r\n|\n|\r/g,"<br>")),d));e.remove();E('<i id="'+g+'"></i>');a.scrollTop(c);var g=f("#"+g),p=g.offset().top-a.offset().top,l=a.height();(c+p>=l||c>p)&&a.scrollTop(c+p-2*l/3);g.remove();
h=!1},200)}).on("@emojibtn.click",function(a){G(g[0]);E(I(a.children().data("name"),'<img alt="{alt}" class="emojione'+(d.sprite?'-{uni}" src="'+M+'">':'" src="{img}">')))}).on("@area.mousedown",function(b,c){!a.autoHideFilters&&!e.is(".focused")&&b.focus();c.preventDefault();return!1}).on("@change",function(a){a=a.html().replace(/<\/?(?:div|span|p)[^>]*>/ig,"");(!a.length||/^<br[^>]*>$/i.test(a))&&d.setText("",!1);c[p](d.getText())}).on("@focus",function(){w();y=P(w,500);e.addClass("focused");a.autoHideFilters&&
l.slideDown(400)}).on("@blur",function(c){n=0;b();e.removeClass("focused");Q(y);a.autoHideFilters&&l.slideUp(400);z.filter(".active").removeClass("active");v.children().hide();c=c.html();d.content!==c&&(d.content=c,F(d,"change",[g]))})}var M="data:image/gif;base64,R0lGODlhAQABAJH/AP///wAAAMDAwAAAACH5BAEAAAIALAAAAAABAAEAAAICVAEAOw==",U={template:"<editor/><filters/><tabs/>",dir:"ltr",spellcheck:!1,autocomplete:"off",autocorrect:"off",autocapitalize:"off",placeholder:null,container:null,hideSource:!0,
autoHideFilters:!1,useSprite:!0,filters:{people:{icon:"yum",emoji:"grinning,grin,joy,smiley,smile,sweat_smile,laughing,innocent,smiling_imp,imp,wink,blush,relaxed,yum,relieved,heart_eyes,sunglasses,smirk,neutral_face,expressionless,unamused,sweat,pensive,confused,confounded,kissing,kissing_heart,kissing_smiling_eyes,kissing_closed_eyes,stuck_out_tongue,stuck_out_tongue_winking_eye,stuck_out_tongue_closed_eyes,disappointed,worried,angry,rage,cry,persevere,triumph,disappointed_relieved,frowning,anguished,fearful,weary,sleepy,tired_face,grimacing,sob,open_mouth,hushed,cold_sweat,scream,astonished,flushed,sleeping,dizzy_face,no_mouth,mask,slight_frown,slight_smile,smile_cat,joy_cat,smiley_cat,heart_eyes_cat,smirk_cat,kissing_cat,pouting_cat,crying_cat_face,scream_cat,footprints,bust_in_silhouette,busts_in_silhouette,levitate,spy,baby,boy,girl,man,woman,family,family_mwg,family_mwgb,family_mwbb,family_mwgg,family_wwb,family_wwg,family_wwgb,family_wwbb,family_wwgg,family_mmb,family_mmg,family_mmgb,family_mmbb,family_mmgg,couple,two_men_holding_hands,two_women_holding_hands,dancers,bride_with_veil,person_with_blond_hair,man_with_gua_pi_mao,man_with_turban,older_man,older_woman,cop,construction_worker,princess,guardsman,angel,santa,ghost,japanese_ogre,japanese_goblin,poop,skull,alien,space_invader,bow,information_desk_person,no_good,ok_woman,raising_hand,person_with_pouting_face,person_frowning,massage,haircut,couple_with_heart,couple_ww,couple_mm,couplekiss,kiss_ww,kiss_mm,raised_hands,clap,ear,eye,eyes,nose,lips,kiss,tongue,nail_care,wave,thumbsup,thumbsdown,point_up,point_up_2,point_down,point_left,point_right,ok_hand,v,punch,fist,raised_hand,muscle,open_hands,writing_hand,hand_splayed,middle_finger,vulcan,pray"},
...
K=[].slice,k=q.emojione||!1,G,J,u={},m={},P=q.setInterval,Q=q.clearInterval,y=[],O;k||f.getScript("https://cdn.jsdelivr.net/emojione/1.5.0/lib/js/emojione.min.js",function(){k=q.emojione;k.imagePathPNG="https://cdnjs.cloudflare.com/ajax/libs/emojione/1.5.0/assets/png/";h.createStyleSheet?h.createStyleSheet("https://cdnjs.cloudflare.com/ajax/libs/emojione/1.5.0/assets/sprites/emojione.sprites.css"):f("<link/>",{rel:"stylesheet",href:"https://cdnjs.cloudflare.com/ajax/libs/emojione/1.5.0/assets/sprites/emojione.sprites.css"}).appendTo("head");
f.each(y,function(c,a){a()})});w(function(){f.each(k.emojioneList,function(c,a){u[c.replace("-","_")]=a});O=RegExp("<object[^>]*>.*?</object>|<span[^>]*>.*?</span>|<(?:object|embed|svg|img|div|span|p|a)[^>]*>|("+k.unicodeRegexp+")","gi")});q.getSelection&&h.createRange?(G=function(c){var a=q.getSelection().getRangeAt(0),b=a.cloneRange();b.selectNodeContents(c);b.setEnd(a.startContainer,a.startOffset);c=b.toString().length;return{start:c,end:c+H(f("<div/>").append(a.extractContents()).html()).length}},
J=function(c,a){var b=0,d=h.createRange();d.setStart(c,0);d.collapse(!0);for(var f=[c],e,k=!1,m=!1;!m&&(e=f.pop());)if(3==e.nodeType){var g=b+e.length;!k&&(a.start>=b&&a.start<=g)&&(d.setStart(e,a.start-b),k=!0);k&&(a.end>=b&&a.end<=g)&&(d.setEnd(e,a.end-b),m=!0);b=g}else for(g=e.childNodes.length;g--;)f.push(e.childNodes[g]);b=q.getSelection();b.removeAllRanges();b.addRange(d)}):h.selection&&h.body.createTextRange&&(G=function(c){var a=h.selection.createRange(),b=h.body.createTextRange();b.moveToElementText(c);
b.setEndPoint("EndToStart",a);c=b.text.length;return{start:c,end:c+a.text.length}},J=function(c,a){var b=h.body.createTextRange();b.moveToElementText(c);b.collapse(!0);b.moveEnd("character",a.end);b.moveStart("character",a.start);b.select()});var t=function(c,a){var b=this;w(function(){T.apply(b,[c,a])})};t.prototype.on=function(c,a){var b=this.id;c&&f.isFunction(a)&&f.each(c.toLowerCase().split(" "),function(c,f){m[b][f]||(m[b][f]=[]);m[b][f].push(a)});return this};t.prototype.off=function(c,a){var b=
this.id,d=/^@/;c&&f.each(c.toLowerCase().split(" "),function(c,e){m[b][e]&&!d.test(e)&&(a?f.each(m[b][e],function(c,d){d===a&&(m[b][e]=m[b][e].splice(c,1))}):m[b][e]=[])});return this};t.prototype.setText=function(c){var a=this,b=arguments;w(function(){a.editor.html(L(c,a));a.content=a.editor.html();1===b.length&&F(a,"change",[a.editor])})};t.prototype.getText=function(){return H(this.editor.html())};f.fn.emojioneArea=function(c){return this.each(function(){return this.emojioneArea?this.emojioneArea:
this.emojioneArea=new t(f(this),c)})}})(document,window,jQuery);
