(function(h,r,d,l){function t(b){var c,a;if(r.getSelection){if(c=r.getSelection(),c.getRangeAt&&c.rangeCount){a=c.getRangeAt(0);a.deleteContents();var f=h.createElement("div");f.innerHTML=b;b=h.createDocumentFragment();for(var d,e;d=f.firstChild;)e=b.appendChild(d);a.insertNode(b);e&&(a=a.cloneRange(),a.setStartAfter(e),a.collapse(!0),c.removeAllRanges(),c.addRange(a))}}else h.selection&&"Control"!=h.selection.type&&h.selection.createRange().pasteHTML(b)}function w(b){var c=this,a=!0,f;b&&(f=x.call(arguments,
1),d.each(b.toLowerCase().split(" "),function(b,e){for(var y=0;1>=y;y++){var g=0==y?"@"+e:e;k[c.id][g]&&k[c.id][g].length&&d.each(k[c.id][g],function(b,e){return a=!1!==e.apply(c,f)});if(!a)break}return a}));return a}function E(b,c){return d(c.currentTarget)}function q(b,c,a,f){function m(a,c,e){a.on(c,function(){var a=d.isFunction(f)?f.apply(b,[c].concat(x.call(arguments))):f;a&&w.apply(b,[e].concat([a]).concat(x.call(arguments)))})}function e(c){"string"===(typeof a).toLowerCase()?m(c,a,a):d.each(a,
function(b,f){m(c,d.isArray(a)?f:b,f)})}f=f||E;d.isArray(c)?d.each(c,function(a,c){e(d(c))}):e(c)}function D(b){return F(b.replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/(?:\r\n|\r|\n)/g,"\n").replace(/(\n+)/g,"<div>$1</div>").replace(/\n/g,"<br/>").replace(/<br\/><\/div>/g,"</div>")).replace(/\t/g,"&nbsp;&nbsp;&nbsp;&nbsp;").replace(/  /g,"&nbsp;&nbsp;")}function z(b){return b.replace(/<img[^>]*alt="([^"]+)"[^>]*>/ig,"$1").replace(/\n|\r/g,"").replace(/<br[^>]*>/ig,"\n").replace(/(?:<(?:div|p|ol|ul|li|pre|code|object)[^>]*>)+/ig,
...
" &nbsp;")}function F(b){return b.replace(RegExp("<object[^>]*>.*?</object>|<span[^>]*>.*?</span>|<(?:object|embed|svg|img|div|span|p|a)[^>]*>|("+l.unicodeRegexp+")","gi"),function(c){if("undefined"===typeof c||""===c||!(c in l.jsecapeMap))return c;c=l.jsecapeMap[c];return'<img class="emojione" alt="'+l.convert(c)+'" src="'+l.imagePathPNG+c+".png"+l.cacheBustParam+'"/>'})}function A(b,c){return b.replace(/:?[\w_]+:?,?/g,function(a){a=":"+a.replace(/,$/,"").replace(/:$/,"").replace(/^:/,"")+":";if(a in
u){var b=u[a][u[a].length-1].toUpperCase(),d=l.convert(b);return c.replace("{shortname}",a).replace("{imagePng}",l.imagePathPNG+b+".png"+l.cacheBustParam).replace("{unicode}",b).replace("{alt}",d)}return a})}function G(b,c){c=d.extend({},H,c);var a=this,f,m,e,h=b.is("TEXTAREA")||b.is("INPUT")?"val":"text",g=c.template,l=!1,p=c.container?d(c.container):!1;a.placeholder=c.placeholder||b.data("placeholder")||b.attr("placeholder")||"";k[a.id]={};for(var s=["editor","filters","tabs"],n=0;3>n;n++)g=g.replace(RegExp("<"+
s[n]+"/?>","i"),'<div class="emojionearea-'+s[n]+'"></div>');g=d("<div>"+g+"</div>").addClass("emojionearea").attr("role","application");this.editor=g.find(".emojionearea-editor").attr("contenteditable","true").attr("placeholder",a.placeholder).attr("tabindex",0);f=a.editor;s=["dir","spellcheck","autocomplete","autocorrect","autocapitalize"];for(n=0;5>n;n++)f.attr(s[n],c[s[n]]);m=g.find(".emojionearea-filters");c.autoHideFilters&&m.hide();e=g.find(".emojionearea-tabs");d.each(c.filters,function(a,
c){d("<span/>").wrapInner(A(c.icon,'<span class="emojione-{unicode}">{alt}</span>')).addClass("emojionearea-filter").attr("data-filter",a).attr("role","button").appendTo(m);d("<div/>").wrapInner(A(c.emoji,'<span class="emojibtn"><span class="emojione-{unicode}" data-shortname="{shortname}">{alt}</span></span>')).addClass("emojionearea-tab").addClass("emojionearea-tab-"+a).hide().appendTo(e)});p?p.replaceWith(g):g.insertAfter(b);c.hideSource&&b.hide();a.setText(b[h]());q(a,[m,e],{mousedown:"area.mousedown"},
...
a.data("filter")).show())}).on("@paste",function(a){l=!0;t("<span>&nbsp;</span>");var c=v(a[0]),b=B.scrollTop(),e=a.scrollTop(),f=d("<div/>").appendTo(d("BODY")).attr("contenteditable","true").attr("tabindex","0").css({position:"fixed",left:"-9999px",width:"1px",height:"1px",top:10}).focus();B.scrollTop(b);a.removeAttr("contenteditable");r.setTimeout(function(){var m="caret-"+(new Date).getTime();a.attr("contenteditable","true").focus();C(a[0],c);t(D(z(f.html().replace(/\r\n|\n|\r/g,"<br>"))));f.remove();
t('<span id="'+m+'"></span>');B.scrollTop(b);a.scrollTop(e);var m=d("#"+m),g=m.offset().top-a.offset().top,h=a.height();(e+g>=h||e>g)&&a.scrollTop(e+g-2*h/3);m.remove();l=!1},200)}).on("@emojibtn.click",function(a){v(f[0]);t(A(a.children("span").data("shortname"),'<img class="emojione" alt="{alt}" src="{imagePng}"/>'))}).on("@area.mousedown",function(a,b){!c.autoHideFilters&&!g.is(".focused")&&a.focus();b.preventDefault();return!1}).on("@change",function(){var a=f.html();/^<br[^>]*>$/.test(a.replace(/<\/?div[^>]*>/g,
""))&&this.setText("");b[h](this.getText())}).on("@focus",function(){g.addClass("focused");c.autoHideFilters&&m.slideDown(400)}).on("@blur",function(b){g.removeClass("focused");c.autoHideFilters&&m.slideUp(400);m.find(".active").removeClass("active");e.children().hide();b=b.html();a.content!==b&&(a.content=b,w.apply(a,["change",a.content]))})}var H={template:"<editor/><filters/><tabs/>",dir:"ltr",spellcheck:!0,autocomplete:"off",autocorrect:"off",autocapitalize:"off",placeholder:null,container:null,
...
x=[].slice,v,C,u={},B=d(h),k={};d.each(l.emojioneList,function(b,c){u[b.replace("-","_")]=c});r.getSelection&&h.createRange?(v=function(b){var c=r.getSelection().getRangeAt(0),a=c.cloneRange();a.selectNodeContents(b);a.setEnd(c.startContainer,c.startOffset);b=a.toString().length;return{start:b,end:b+z(d("<div/>").append(c.extractContents()).html()).length}},C=function(b,c){var a=0,f=h.createRange();f.setStart(b,0);f.collapse(!0);for(var d=[b],e,l=!1,g=!1;!g&&(e=d.pop());)if(3==e.nodeType){var k=a+
e.length;!l&&(c.start>=a&&c.start<=k)&&(f.setStart(e,c.start-a),l=!0);l&&(c.end>=a&&c.end<=k)&&(f.setEnd(e,c.end-a),g=!0);a=k}else for(k=e.childNodes.length;k--;)d.push(e.childNodes[k]);a=r.getSelection();a.removeAllRanges();a.addRange(f)}):h.selection&&h.body.createTextRange&&(v=function(b){var c=h.selection.createRange(),a=h.body.createTextRange();a.moveToElementText(b);a.setEndPoint("EndToStart",c);b=a.text.length;return{start:b,end:b+c.text.length}},C=function(b,c){var a=h.body.createTextRange();
a.moveToElementText(b);a.collapse(!0);a.moveEnd("character",c.end);a.moveStart("character",c.start);a.select()});var p=function(b,c){this.id=(new Date).getTime();G.apply(this,[d(b),c])};p.prototype.on=function(b,c){var a=this;b&&d.isFunction(c)&&d.each(b.toLowerCase().split(" "),function(b,d){k[a.id][d]||(k[a.id][d]=[]);k[a.id][d].push(c)});return a};p.prototype.off=function(b,c){var a=this,f=/^@/;b&&(d.isFunction(c)?d.each(b.toLowerCase().split(" "),function(b,e){k[a.id][e]&&!f.test(e)&&d.each(k[a.id][e],
function(b,d){d===c&&(k[a.id][e]=k[a.id][e].splice(b,1))})}):d.each(b.split(" "),function(c,b){f.test(b)||(k[a.id][b]=[])}));return a};p.prototype.setText=function(b){this.editor.html('<div class="placeholder-fix">'+D(b)+"</div>");this.placeholder&&this.editor.children(".placeholder-fix:first").attr("placeholder",this.placeholder);this.content=this.editor.html();w.apply(this,["change",this.content])};p.prototype.getText=function(){return z(this.editor.html())};d.fn.emojioneArea=function(b){return this.each(function(){return this.emojioneArea?
this.emojioneArea:this.emojioneArea=new p(d(this),b)})}})(document,window,jQuery,emojione);
